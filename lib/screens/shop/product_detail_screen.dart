...
class _ProductDetailScreenState extends State<ProductDetailScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _variantsController = AnimationController(duration: const Duration(milliseconds: 300), vsync: this);
    _variantsAnimation = CurvedAnimation(parent: _variantsController, curve: Curves.easeOut);
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gagal memuat produk.')));
      }
    } finally {
  ...
  void _addToCart({bool buyNow = false}) {
    ...
    final cartKey = variant != null ? '${product.id}:${variant.id}' : '${product.id}:base';
    final itemName = variant != null ? '${product.name} (${variant.displayName})' : product.name;
    final price = variant?.price ?? product.price;
    final stock = variant?.stock ?? product.stock;

    if (product.type == 'physical' && stock <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('Stok habis!'), backgroundColor: Colors.red,
    ...
    final item = CartItemModel(
      id: product.id, productId: product.id, cartKey: cartKey,
      name: itemName, baseName: product.name,
      variantId: variant?.id, variantLabel: variant?.displayName,
      attributeName: variant?.attributeName, attributeValue: variant?.attributeValue,
      price: price, imageUrl: product.imageUrl, type: product.type, stock: stock, quantity: 1,
    );
    ...
    context.read<CartProvider>().addToCart(item).then((ok) {
      ...
      if (ok) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Stok tidak mencukupi!'), backgroundColor: Colors.red,
              behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
        );
      }
    });
                              ...
                              child: Column(
                                ...
                                children: [
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Text(
                                          ...
                                          style: const TextStyle(fontSize: 10, color: AppColors.primaryBlue, fontWeight: FontWeight.w700),
                                        ),
                                      ),
                                      if (_product!.type == 'digital' && _product!.durationDays != null) ...[
                                  ...
                                  Text(_formatPrice(_displayPrice),
                                      style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: AppColors.primaryBlue)),
                                  if (_product!.description != null) ...[
                                    const SizedBox(height: 16),
                                    const Text('Deskripsi', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                                  ...
                                  if (_product!.hasVariants) ...[
                                    ...
                                    Row(
                                      children: [
                                        const Text('Pilih Varian', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                                        const SizedBox(width: 8),
                                        if (_selectedVariant == null)
                                          Container(
                                            ...
                                            child: const Text('Wajib dipilih', style: TextStyle(fontSize: 10, color: Colors.orange, fontWeight: FontWeight.w600)),
                                          ),
                                      ],
                                    ),
                                    const SizedBox(height: 12),
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  if (v.stock <= 5 && v.stock > 0)
                                                    Text('Sisa ${v.stock}', style: TextStyle(fontSize: 9, color: isSelected ? Colors.white70 : Colors.orange)),
                                                  if (v.stock <= 0)
                                                    Text('Habis', style: TextStyle(fontSize: 9, color: isSelected ? Colors.white54 : Colors.red)),
                                                ],
                                              ),
                                            ),
                                          );
                                  ...
                                  const SizedBox(height: 16),
                                  Row(children: [
                                    Icon(Icons.inventory_2_outlined, size: 16, color: AppColors.textSecondary.withOpacity(0.7)),
                          ...
                          children: [
                            Expanded(
                              child: OutlinedButton(
                                ...
                                child: const Text('+ Keranjang', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.primaryBlue)),
                              ),
                            ),
                            ...
                            Expanded(
                              child: ElevatedButton(
                                ...
                                child: const Text('Beli Sekarang', style: TextStyle(fontWeight: FontWeight.w700, color: Colors.white)),
                              ),
                            ),
                          ],
                        ),
